...
### Dropping "bad" data\
I was having trouble removing "bad" data from teh combined data frame so I will 
drop "bad" data from each data frame prior to combing them into a single data
frame.\

There are two scenarios where data is considered "bad". The first is
when bikes were removed from service for a testing. This is denoted in the
'start_station_name' column by 'WATSON TESTING - DIVVY'. The second is when 
the 'ride_length' is negative. More on the second scenario in a later step.\

In the code chunk below I create new dataframes by using operators to 
select all records that do NOT include 'WATSON TESTING - DIVVY' in the 'start_station_name' column 
and then drop rows containing all NAs. The new dataframes is are v2 and v3 for each month.
```{r}
m10_2020_v2 <- m10_2020[!(m10_2020$start_station_name == "WATSON TESTING - DIVVY"),]
m10_2020_v3 <-  m10_2020_v2[rowSums(is.na(m10_2020_v2[ , 0:ncol(m10_2020_v2)])) < ncol(m10_2020_v2), ]

m11_2020_v2 <- m11_2020[!(m11_2020$start_station_name == "WATSON TESTING - DIVVY"),]
m11_2020_v3 <-  m11_2020_v2[rowSums(is.na(m11_2020_v2[ , 0:ncol(m11_2020_v2)])) < ncol(m11_2020_v2), ]

m12_2020_v2 <- m12_2020[!(m12_2020$start_station_name == "WATSON TESTING - DIVVY"),]
m12_2020_v3 <-  m12_2020_v2[rowSums(is.na(m12_2020_v2[ , 0:ncol(m12_2020_v2)])) < ncol(m12_2020_v2), ]

m01_2021_v2 <- m01_2021[!(m01_2021$start_station_name == "WATSON TESTING - DIVVY"),]
m01_2021_v3 <-  m01_2021_v2[rowSums(is.na(m01_2021_v2[ , 0:ncol(m01_2021_v2)])) < ncol(m01_2021_v2), ]

m02_2021_v2 <- m02_2021[!(m02_2021$start_station_name == "WATSON TESTING - DIVVY"),]
m02_2021_v3 <-  m02_2021_v2[rowSums(is.na(m02_2021_v2[ , 0:ncol(m02_2021_v2)])) < ncol(m02_2021_v2), ]

m03_2021_v2 <- m03_2021[!(m03_2021$start_station_name == "WATSON TESTING - DIVVY"),]
m03_2021_v3 <-  m03_2021_v2[rowSums(is.na(m03_2021_v2[ , 0:ncol(m03_2021_v2)])) < ncol(m03_2021_v2), ]

m04_2021_v2 <- m04_2021[!(m04_2021$start_station_name == "WATSON TESTING - DIVVY"),]
m04_2021_v3 <-  m04_2021_v2[rowSums(is.na(m04_2021_v2[ , 0:ncol(m04_2021_v2)])) < ncol(m04_2021_v2), ]

m05_2021_v2 <- m05_2021[!(m05_2021$start_station_name == "WATSON TESTING - DIVVY"),]
m05_2021_v3 <-  m05_2021_v2[rowSums(is.na(m05_2021_v2[ , 0:ncol(m05_2021_v2)])) < ncol(m05_2021_v2), ]

m06_2021_v2 <- m06_2021[!(m06_2021$start_station_name == "WATSON TESTING - DIVVY"),]
m06_2021_v3 <-  m06_2021_v2[rowSums(is.na(m06_2021_v2[ , 0:ncol(m06_2021_v2)])) < ncol(m06_2021_v2), ]

m07_2021_v2 <- m07_2021[!(m07_2021$start_station_name == "WATSON TESTING - DIVVY"),]
m07_2021_v3 <-  m07_2021_v2[rowSums(is.na(m07_2021_v2[ , 0:ncol(m07_2021_v2)])) < ncol(m07_2021_v2), ]

m08_2021_v2 <- m08_2021[!(m08_2021$start_station_name == "WATSON TESTING - DIVVY"),]
m08_2021_v3 <-  m08_2021_v2[rowSums(is.na(m08_2021_v2[ , 0:ncol(m08_2021_v2)])) < ncol(m08_2021_v2), ]

m09_2021_v2 <- m09_2021[!(m09_2021$start_station_name == "WATSON TESTING - DIVVY"),]
m09_2021_v3 <-  m09_2021_v2[rowSums(is.na(m09_2021_v2[ , 0:ncol(m09_2021_v2)])) < ncol(m09_2021_v2), ]
```

The amount of "bad" testing and NA data removed included 523,467 records or 
approximately 0.1% of the total records.
...
### Dropping "bad" data\
One last step in the data wrangling, merging, and cleaning process, removing the last of the
"bad" data. This is where I will drop 'ride_length' data that is negative.

In the code chunk below I create a new dataframe by using operators to 
select all records that have a 'ride_length' less than zero (0). The new dataframe is called all_trips_v2.
```{r new df}
